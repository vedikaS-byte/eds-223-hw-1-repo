...
Research question: To what extent are low-income people of color in Nueces County within distance of a hazardous waste facility?
...
# map of concentrations of POC related to # of hardaous waste facilities in the block group 
# nueces_id <- nueces_county %>% filter(ID == "483550005001")

tm_shape(travis_county, bbox = st_bbox(travis_county)) + 
  #tm_fill() + 
tm_polygons("PEOPCOLORPCT", fill.scale = tm_scale(values = "blue"), fill.legend = tm_legend(title = "Percentage of People of Color")) + tm_symbols("PTSDF", title.size = "Hazardous Waste Proximity (km)")+ tm_grid(
    col = "black", lwd = 0.5, 
    alpha = 0.7
  ) +
  tm_graticules(
    ...
    position = c("top", "center"), text.size = 0.7, size = 1)+ 
  tm_layout(
  title.size = 2)
...
tm_shape(travis_county) +
  ...
  # ) +
  tm_layout(
    title = "Hazardous Waste Proximity EJ Index to  Communities in Travis County",
    title.size = 8,
